<template>
  <div>
    ...
    <el-table :data="tableData"
              ...
              border>
      <el-table-column prop="teachingEvaluationId" label="ID" width="60"></el-table-column>
      ...
      <el-table-column prop="evaluationContent" label="评教内容"></el-table-column>
      <el-table-column prop="operate" label="操作" width="200">
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="mod(scope.row)">编辑</el-button>
          <el-popconfirm
              title="确定删除吗？"
              @confirm="del(scope.row.teachingEvaluationId)"
    ...
    <el-dialog
      ...
      <el-form ref="form" :rules="rules" :model="form" label-width="120px">
        ...
        <el-form-item label="评教内容" prop="evaluationContent">
...
export default {
  ...
  methods: {
    ...
    del(id) {
      this.$axios.get(this.$httpUrl + '/evaluation/del?id=' + id).then(res => res.data).then(res => {
        if (res.code == 200) {
          this.$message({
            message: '操作成功！',
            type: 'success'
    ...
    doSave() {
      this.$axios.post(this.$httpUrl + '/evaluation/save', this.form).then(res => res.data).then(res => {
        if (res.code == 200) {
          this.$message({
            message: '操作成功！',
            type: 'success'
    ...
    doMod() {
      this.$axios.post(this.$httpUrl + '/evaluation/update', this.form).then(res => res.data).then(res => {
        if (res.code == 200) {
          this.$message({
            message: '操作成功！',
            type: 'success'
    ...
    save() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          if (this.form.teachingEvaluationId) {
            this.doMod();
          } else {
            this.doSave();
          }
    ...
    loadPost() {
      this.$axios.post(this.$httpUrl + '/evaluation/listPageC1', {
